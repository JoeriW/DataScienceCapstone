...
dlPath <- "./download"
dlFile <- paste(dlPath,"swift.zip",sep="/")
exPath <- "./extract"
finalPath <-"./extract/final/en_US"

if(!file.exists(dlPath)){dir.create(dlPath)}
if(!file.exists(dlFile)){download.file(url=url,destfile=dlFile)}
if(!file.exists(finalPath)){unzip(zipfile=dlFile,exdir=exPath)}

#Loading the data
library(tm)

enCorpus <- Corpus(DirSource(finalPath,encoding = "UTF-8"),
...
enLines <- NULL
enWords <- NULL
enCharactersWord <- NULL
enWordsLine <- NULL

for (i in 1:length(enCorpus)){
  
  enCharacters[[i]] <- sum(nchar(enCorpus[[i]]$content))
  
  enLines[[i]] <- length(enCorpus[[i]]$content)
  
  enWords[[i]] <- sum(stri_count_words(enCorpus[[i]]$content))
  
  enCharactersWord[[i]] <- log(nchar(enCorpus[[i]]$content)/stri_count_words(enCorpus[[i]]$content))
  
  enWordsLine[[i]] <- log(stri_count_words(enCorpus[[i]]$content))
}

enDf <- data.frame(c("Blogs","News","Twitter"),
                   formatC(enLines,big.mark=","),
                   formatC(enWords,big.mark=","),
                   formatC(enCharacters,big.mark=","),
                   round(enWords/enLines,0),
                   round(enCharacters/enWords,1)
                   )
...
| `r names(enDf[1])` | `r names(enDf[2])` | `r names(enDf[3])` | `r names(enDf[4])` | `r names(enDf[5])` | `r names(enDf[6])` |
|:------------------:|:------------------:|:------------------:|:------------------:|:------------------:|:------------------:|
|    `r enDf[1,1]`   |    `r enDf[1,2]`   |    `r enDf[1,3]`   |    `r enDf[1,4]`   |    `r enDf[1,5]`   |    `r enDf[1,6]`   |
|    `r enDf[2,1]`   |    `r enDf[2,2]`   |    `r enDf[2,3]`   |    `r enDf[2,4]`   |    `r enDf[2,5]`   |    `r enDf[2,6]`   |
|    `r enDf[3,1]`   |    `r enDf[3,2]`   |    `r enDf[3,3]`   |    `r enDf[3,4]`   |    `r enDf[3,5]`   |    `r enDf[3,6]`   |


The number words per line is considerably lower for Twitter updates (`r enDf[3,5]`) compared to the words per line of text in newspapers (`r enDf[2,5]`) and blogs (`r enDf[1,5]`). This shouldn't be a surprise given that Twitter limits the number of characters per tweet to 140. The fact that blogs have a higher number of words per line compared to newspaper articles intuitively makes sense given that newspaper articles often publish information in a concise way, whereas writing style for blogs is often more prosaic, with use of longer sentences. 
...
boxplot(enWordsLine,
        main = "# words/line",
        names= enDf$source,
        ylab = "log(words per line)"
        )
boxplot(enCharactersWord,
        main = "# characters / word",
        names= enDf$source,
        ylab = "log(characters per word)"
        )
...
rate=0.05
for (i in 1:length(enCorpus)){   
  y <- sample(1:length(enCorpus[[i]]$content)
              ,round(rate*length(enCorpus[[i]]$content))
              ,replace=F
  )
  enCorpus[[i]]$content <- enCorpus[[i]]$content[y]
}
```

Our sample will be **`r percent(rate)`** of the initial data, randomly selected across the whole dataset.
...
enCorpus <- tm_map(enCorpus,content_transformer(function(x)gsub("\u2019|\u0060", "'", x)))
enCorpus <- tm_map(enCorpus,content_transformer(function(x)gsub("&", "and", x)))# & to and
enCorpus <- tm_map(enCorpus,content_transformer(function(x)gsub("$", "dollar", x)))# $ to dollar
enCorpus <- tm_map(enCorpus,content_transformer(function(x)gsub("=", "is", x)))# = to is
enCorpus <- tm_map(enCorpus,content_transformer(function(x)gsub("[\u0023|\u002A|\u003B|\u003A|\u0028|\u0029|\u002C|\u002E|\u003F|\u0021|\u201D|\u201C|\u002F|\u00B8|\u0153]","", x)))
enCorpus <- tm_map(enCorpus,content_transformer(function(x) gsub("[\u002D|\u207B|\u208B|\uFE63|\uFF0D|\uE002D|\u00BD|\u005C|\u0022|\u0040|\u003C|\u003E|\u005F|\u00ED]", "", x)))
enCorpus <- tm_map(enCorpus,content_transformer(function(x)gsub("[\u1F466]","",x)))
enCorpus <- tm_map(enCorpus, content_transformer(function(x)stri_replace_all(x,regex="[^[:graph:]]",replacement=" ")))
enCorpus <- tm_map(enCorpus, content_transformer(function(x)stri_trans_tolower(x)))
enCorpus <- tm_map(enCorpus,content_transformer(function(x) gsub("^\\s+|\\s+$", "", x)))
```
...
```{r,echo=F,warning=FALSE,message=F}
url="https://github.com/JoeriW/DataScienceCapstone/blob/master/download/profanity.csv"
dlFile <- paste(dlPath,"",sep="/")
finalPath <-"./download/profanity.csv"

if(!file.exists(finalPath)){download.file(url=url,destfile=dlFile)}

bannedWords <-read.csv(finalPath,
                       header=F,
                       sep=",")

enCorpus <- tm_map(enCorpus,removeWords,bannedWords[,1])
```
...
oneGrTok <- function(x) NGramTokenizer(x, Weka_control(delimiters= " \\t\\r\\n.!?,;\"()",min = 1, max = 1))
oneGrCorpus <- TermDocumentMatrix(enCorpus,
                                  control=list(tokenize=oneGrTok,
                                                wordLengths=c(0,Inf)
                                                )
                                  )
```
...
for (i in 1:ncol(x)){
        ...
        y <- as.matrix(x[,i])  
        z <- y[head(order(-y),75),,drop=F]
        wordcloud(words=row.names(z),
                  freq=z,
                  random.order=T,
...
oneGrCorpusStopRm <- TermDocumentMatrix(enCorpus,
                                  control=list(tokenize=oneGrTok,
                                               wordLengths=c(0,Inf),
                                               stopwords = T 
                                                )
...
freq_barplot <- function(GrCorpus,title){
        GrMatrix <- head(sort(rowSums(as.matrix(GrCorpus)),decreasing = TRUE),10)
        GrMatrix <- data.frame("word" = names(GrMatrix), "freq" = GrMatrix)
        rownames(GrMatrix)<- NULL
        barplot(GrMatrix$freq,names.arg = GrMatrix$word,col="lightblue",las=2,main=title,ylab="frequency")
        }

twoGrTok <- function(x) NGramTokenizer(x, Weka_control(delimiters= " \\t\\r\\n.!?,;\"()",min = 2, max = 2))
twoGrCorpus <- TermDocumentMatrix(enCorpus,
                                  control=list(tokenize=twoGrTok,
                                               wordLengths=c(0,Inf)
                                               )
                                  )
freq_barplot(twoGrCorpus,"top 10 2-grams")
rm(twoGrCorpus)

threeGrTok <- function(x) NGramTokenizer(x, Weka_control(delimiters= " \\t\\r\\n.!?,;\"()",min = 3, max = 3))
threeGrCorpus <- TermDocumentMatrix(enCorpus,
                                  control=list(tokenize=threeGrTok,
                                               wordLengths=c(0,Inf)
                                               )
                                  )
freq_barplot(threeGrCorpus,"top 10 3-grams")
rm(threeGrCorpus)

fourGrTok <- function(x) NGramTokenizer(x, Weka_control(delimiters= " \\t\\r\\n.!?,;\"()",min = 4, max = 4))
fourGrCorpus <- TermDocumentMatrix(enCorpus,
                                  control=list(tokenize=fourGrTok,
                                               wordLengths=c(0,Inf)
                                               )
                                  )
freq_barplot(fourGrCorpus,"top 10 4-grams")
rm(fourGrCorpus)
```
